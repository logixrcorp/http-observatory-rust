...
impl HstsPreload {
    ...
    pub fn is_preloaded(&self, domain: &str) -> bool {
        ...
        // Check parents for include_subdomains
        let mut current_domain = domain;
        while let Some(dot_index) = current_domain.find('.') {
            if dot_index + 1 >= current_domain.len() {
                break;
            }
            current_domain = &current_domain[dot_index + 1..];
            
            if let Some(&include_subdomains) = self.domains.get(current_domain) {
